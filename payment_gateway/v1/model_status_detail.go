// Copyright 2026 PT Espay Debit Indonesia Koe
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package payment_gateway

import (
	"encoding/json"
	"fmt"
	"bytes"
	utils "github.com/dana-id/dana-go/v2/utils"
)

// checks if the StatusDetail type satisfies the MappedNullable interface at compile time
var _ utils.MappedNullable = &StatusDetail{}

// StatusDetail struct for StatusDetail
type StatusDetail struct {
	// Acquirement status. The enums:<br> * INIT - Order is created but not paid yet<br> * SUCCESS - Order is succeeded<br> * CLOSED - Order is closed<br> * PAYING - Order is paid but not finish<br> * MERCHANT_ACCEPT - Order is accepted by merchant after order is paid for PAY-CONFIRM<br> * CANCELLED - Order is cancelled<br> 
	AcquirementStatus string `json:"acquirementStatus"`
	// Whether the frozen is true or not
	Frozen *string `json:"frozen,omitempty"`
	// Whether the cancelled is true or not
	Cancelled *string `json:"cancelled,omitempty"`
}

type _StatusDetail StatusDetail

// NewStatusDetail instantiates a new StatusDetail object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStatusDetail(acquirementStatus string) *StatusDetail {
	this := StatusDetail{}
	this.AcquirementStatus = acquirementStatus
	return &this
}

// NewStatusDetailWithDefaults instantiates a new StatusDetail object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStatusDetailWithDefaults() *StatusDetail {
	this := StatusDetail{}
	return &this
}

// GetAcquirementStatus returns the AcquirementStatus field value
func (o *StatusDetail) GetAcquirementStatus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.AcquirementStatus
}

// GetAcquirementStatusOk returns a tuple with the AcquirementStatus field value
// and a boolean to check if the value has been set.
func (o *StatusDetail) GetAcquirementStatusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AcquirementStatus, true
}

// SetAcquirementStatus sets field value
func (o *StatusDetail) SetAcquirementStatus(v string) {
	o.AcquirementStatus = v
}

// GetFrozen returns the Frozen field value if set, zero value otherwise.
func (o *StatusDetail) GetFrozen() string {
	if o == nil || utils.IsNil(o.Frozen) {
		var ret string
		return ret
	}
	return *o.Frozen
}

// GetFrozenOk returns a tuple with the Frozen field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StatusDetail) GetFrozenOk() (*string, bool) {
	if o == nil || utils.IsNil(o.Frozen) {
		return nil, false
	}
	return o.Frozen, true
}

// HasFrozen returns a boolean if a field has been set.
func (o *StatusDetail) HasFrozen() bool {
	if o != nil && !utils.IsNil(o.Frozen) {
		return true
	}

	return false
}

// SetFrozen gets a reference to the given string and assigns it to the Frozen field.

func (o *StatusDetail) SetFrozen(v string) error {
	// Validate string length constraints
	if !utils.IsNil(v) {
		if utils.Strlen(v) > 64 {
			return fmt.Errorf("invalid length for Frozen, must be smaller than or equal to 64")
		}
	}
	o.Frozen = &v
	return nil
}




// GetCancelled returns the Cancelled field value if set, zero value otherwise.
func (o *StatusDetail) GetCancelled() string {
	if o == nil || utils.IsNil(o.Cancelled) {
		var ret string
		return ret
	}
	return *o.Cancelled
}

// GetCancelledOk returns a tuple with the Cancelled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StatusDetail) GetCancelledOk() (*string, bool) {
	if o == nil || utils.IsNil(o.Cancelled) {
		return nil, false
	}
	return o.Cancelled, true
}

// HasCancelled returns a boolean if a field has been set.
func (o *StatusDetail) HasCancelled() bool {
	if o != nil && !utils.IsNil(o.Cancelled) {
		return true
	}

	return false
}

// SetCancelled gets a reference to the given string and assigns it to the Cancelled field.

func (o *StatusDetail) SetCancelled(v string) error {
	// Validate string length constraints
	if !utils.IsNil(v) {
		if utils.Strlen(v) > 64 {
			return fmt.Errorf("invalid length for Cancelled, must be smaller than or equal to 64")
		}
	}
	o.Cancelled = &v
	return nil
}




func (o StatusDetail) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o StatusDetail) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["acquirementStatus"] = o.AcquirementStatus
	if !utils.IsNil(o.Frozen) {
		toSerialize["frozen"] = o.Frozen
	}
	if !utils.IsNil(o.Cancelled) {
		toSerialize["cancelled"] = o.Cancelled
	}
	return toSerialize, nil
}

func (o *StatusDetail) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"acquirementStatus",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varStatusDetail := _StatusDetail{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varStatusDetail)

	if err != nil {
		return err
	}

	*o = StatusDetail(varStatusDetail)

	return err
}

type NullableStatusDetail struct {
	value *StatusDetail
	isSet bool
}

func (v NullableStatusDetail) Get() *StatusDetail {
	return v.value
}

func (v *NullableStatusDetail) Set(val *StatusDetail) {
	v.value = val
	v.isSet = true
}

func (v NullableStatusDetail) IsSet() bool {
	return v.isSet
}

func (v *NullableStatusDetail) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStatusDetail(val *StatusDetail) *NullableStatusDetail {
	return &NullableStatusDetail{value: val, isSet: true}
}

func (v NullableStatusDetail) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStatusDetail) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


